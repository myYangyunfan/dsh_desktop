...
test('boot 链一条龙：applyAll(56) → composition-integrity → preflight → 二遍幂等', { skip: !hasPristine() && '缺 .tmp-rc2-stage pristine 源' }, async (t) => {
  const { appDir, home, ctx } = await buildTempRoots(t);

  // ---- 1. 一遍 applyAll：56 补丁全执行、有落盘、零 errors ----
  const r1 = applyAll(ctx);
  assert.equal(r1.total, 56, `注册表应有 56 个补丁（实际 ${r1.total}）`);
  assert.equal(PATCH_SPECS.length, 56, 'PATCH_SPECS 与编排 total 一致');
