...
- **settings.yaml 写后校验与自动回写防护（A-12）**：外壳监听 `~/.dsh/settings.yaml` 变更（1s 防抖 + 5s 失败退避），写后立即校验（必需配置段 `agent-default-model` 存在性，YAML 不可用时行级降级），校验失败自动回写最近可用备份（`userData/settings-guard-backup.json`，原子 tmp+rename），回写后 watcher 幂等重建；读取失败不误回写。新增 `scripts/lib/settings-guard.js` + 10 项单测。
- **运行期轻量化（P2 系列）**：① M3 设置页 observer 按「设置页导航」门控——进入设置页才观察、离开立即断开（不再在会话页承受全文档 MutationObserver 高频回调）；② 删除 3 秒一次的「当前会话」localStorage 轮询上报通道（连同主进程 `dsh:current-session` handler 与调试日志字段，消除固定间隔 IPC + 顺带移除无 sender 校验的通道）；③ 页面异常（window.onerror / unhandledrejection）转发加 5s 同签名节流（与主进程 console-message 同款，异常循环不再淹没 IPC 与日志写入）；④ 标题栏图标 dataURI 进程级缓存（`chrome:init` 每次页面加载/刷新不再 readFileSync icon.png）；⑤ web 启动超时统一为单一常量 `WEB_BOOT_TIMEOUT_MS = 120000`（原先「URL 行等待 60 秒」与「HTTP 就绪探测 120 秒」双标，文案与阈值同源）。
### 修复
